...
def combine_todos_and_calendar(todos, calendar_events, date_label='Today'):
    ...
    for event in calendar_events:
        if event.get('all_day'):
            event['time_label'] = generate_time_label(None, date_label, include_time=False)
        else:
...
def filter_dailies_for_date(all_dailies, target_date, include_carryover=False, carryover_date=None):
    """
    ...
    for daily in all_dailies:
        ...
        if include_carryover:
            if daily.should_complete_on(carryover_date) and not daily.is_completed_on(carryover_date):
...
def get_priority_display(priority_code):
    ...
    return priority_map.get(priority_code, {
...
def get_difficulty_display(difficulty_code):
    ...
    return difficulty_map.get(difficulty_code, {
        'name': difficulty_code.capitalize(),
        'badge_class': 'badge-secondary'
    })
