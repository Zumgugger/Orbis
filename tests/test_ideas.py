...
def test_create_idea(authenticated_client, app, test_user):
    ...
    response = authenticated_client.post('/ideas/create', data={
        'title': 'New Test Idea',
...
def test_save_idea_notes(authenticated_client, app, sample_idea):
    ...
    response = authenticated_client.post(f'/ideas/{sample_idea}/notes', 
...
def test_save_idea_mindmap(authenticated_client, app, sample_idea):
    ...
    response = authenticated_client.post(f'/ideas/{sample_idea}/mindmap',
...
def test_upload_file_to_idea(authenticated_client, app, sample_idea, test_user):
    ...
    response = authenticated_client.post(f'/ideas/{sample_idea}/upload',
        data={'file': test_file},
...
def test_upload_invalid_file_extension(authenticated_client, sample_idea):
    ...
    if response.status_code == 200:
        ...
        assert b'Invalid file' in response.data or b'not allowed' in response.data or b'error' in response.data.lower()
...
def test_delete_file(authenticated_client, app, sample_idea):
    ...
    response = authenticated_client.post(f'/ideas/files/{file_id}/delete', follow_redirects=True)
    assert response.status_code == 200
