...
def test_create_todo(authenticated_client, app, test_user):
    ...
    response = authenticated_client.post('/todos/create', data={
        'title': 'New Test Todo',
...
def test_toggle_todo_status(authenticated_client, app, sample_todo):
    ...
    # Toggle to completed
    response = authenticated_client.post(f'/todos/{sample_todo}/toggle', follow_redirects=True)
    assert response.status_code == 200
...
def test_edit_todo(authenticated_client, app, sample_todo):
    ...
    response = authenticated_client.post(f'/todos/{sample_todo}/edit', data={
...
def test_delete_todo(authenticated_client, app, sample_todo):
    ...
    response = authenticated_client.post(f'/todos/{sample_todo}/delete', follow_redirects=True)
    assert response.status_code == 200
