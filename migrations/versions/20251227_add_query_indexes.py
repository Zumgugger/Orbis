...
def upgrade():
    ...
    def _has_index(table_name, index_name):
        if not insp.has_table(table_name):
            return True
        return any(
            idx.get("name") == index_name for idx in insp.get_indexes(table_name)
        )

    if not _has_index("todos", "ix_todos_user_status_due"):
        op.create_index(
            "ix_todos_user_status_due", "todos", ["user_id", "status", "due_date"]
        )
    ...
    if not _has_index("habits", "ix_habits_user_focus_pos"):
        op.create_index(
            "ix_habits_user_focus_pos", "habits", ["user_id", "focused", "position"]
        )
    ...
    if not _has_index("shopping_lists", "ix_shopping_lists_user_updated"):
        op.create_index(
            "ix_shopping_lists_user_updated",
            "shopping_lists",
            ["user_id", "updated_at"],
        )
    ...
    if not _has_index("master_categories", "ix_master_categories_user_position"):
        op.create_index(
            "ix_master_categories_user_position",
            "master_categories",
            ["user_id", "position"],
        )
    ...
    if not _has_index("master_sections", "ix_master_sections_cat_user_position"):
        op.create_index(
            "ix_master_sections_cat_user_position",
            "master_sections",
            ["category_id", "user_id", "position"],
        )
