...
def _reseq_sections(category_id, user_id):
    secs = MasterSection.query.filter_by(category_id=category_id, user_id=user_id).order_by(MasterSection.position, MasterSection.id).all()
...
def index():
    ...
    sections = []
    if selected_id:
        sections = MasterSection.query.filter_by(category_id=selected_id, user_id=current_user.id).order_by(MasterSection.position, MasterSection.id).all()

    builder_ids = _builder_list()
    builder_sections = []
    if builder_ids:
        builder_sections = (MasterSection.query.filter(MasterSection.user_id == current_user.id, MasterSection.id.in_(builder_ids))
                            .order_by(MasterSection.id).all())
...
def move_category(cat_id, direction):
    cat = MasterCategory.query.filter_by(id=cat_id, user_id=current_user.id).first_or_404()
    cats = MasterCategory.query.filter_by(user_id=current_user.id).order_by(MasterCategory.position, MasterCategory.id).all()
    idx = cats.index(cat)
    if direction == 'up' and idx > 0:
        cats[idx].position, cats[idx-1].position = cats[idx-1].position, cats[idx].position
...
def move_section(sec_id, direction):
    sec = MasterSection.query.filter_by(id=sec_id, user_id=current_user.id).first_or_404()
    secs = MasterSection.query.filter_by(category_id=sec.category_id, user_id=current_user.id).order_by(MasterSection.position, MasterSection.id).all()
    idx = secs.index(sec)
    if direction == 'up' and idx > 0:
        secs[idx].position, secs[idx-1].position = secs[idx-1].position, secs[idx].position
